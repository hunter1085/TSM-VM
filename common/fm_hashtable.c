				 ...
				 zc_hashtable_del_fn value_del)
{
	...
	a_table = fm_calloc(1, sizeof(*a_table));
	if (!a_table) {
		FM_LOGE("fm_calloc fail, errno[%d]", errno);
		return NULL;
	}

	a_table->tab = fm_calloc(a_size, sizeof(*(a_table->tab)));
	if (!a_table->tab) {
		FM_LOGE("fm_calloc fail, errno[%d]", errno);
		fm_free(a_table);
		return NULL;
...
void zc_hashtable_del(zc_hashtable_t * a_table)
{
	...
	if (!a_table) {
		FM_LOGE("a_table[%p] is NULL, just do nothing", a_table);
		return;
	}
...
static int zc_hashtable_rehash(zc_hashtable_t * a_table)
{
	...
	tab = fm_calloc(tab_size, sizeof(*tab));
	if (!tab) {
		FM_LOGE("fm_calloc fail, errno[%d]", errno);
		return -1;
	}
...
void zc_hashtable_remove(zc_hashtable_t * a_table, const void *a_key)
{
	...
	unsigned int i;
        ...
        if (!a_table || !a_key) {
		FM_LOGE("a_table[%p] or a_key[%p] is NULL, just do nothing", a_table, a_key);
		return;
        }
	...
	if (!p) {
		FM_LOGE("p[%p] not found in hashtable", p);
		return;
	}
